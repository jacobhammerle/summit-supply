...
- `work/base-src/` — the app at the PR **base** commit. The bug should exist
  here.
- `work/head-src/` — the app at the PR **head** commit. The fix should exist
  here.
...
1. Read the context. Establish from it: what bug the PR claims to fix, where
   it lives in the app, and what a user would observe.
...
3. Write `work/plan.md` with:
   ...
   - Exact repro steps in the app UI, written for an agent that sees only
     ...
     which values.
   - Expected behavior on the base build (the bug) and on the head build
     (fixed), each stated as an observable fact ("after tapping Save, the
     summary text reads X").
